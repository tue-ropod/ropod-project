...
W_whvec = [];
for i = 1:Nwheels    
    W_whvec = [W_whvec; W_wh(1,i); W_wh(2,i)];
end


Jv = simplify(equationsToMatrix(vI_R,W_whvec));
Jw = simplify(equationsToMatrix(wI_R,W_whvec));
Jddelta = simplify(equationsToMatrix(ddelta,W_whvec));
JW_wh = simplify(equationsToMatrix(W_wh,W_whvec));
Jqw = [Jv; Jw];
